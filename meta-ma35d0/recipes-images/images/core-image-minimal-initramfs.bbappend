DEPENDS += "python3-nuwriter-native mtd-utils-native"

do_deploy[depends] = "virtual/kernel:do_deploy virtual/trusted-firmware-a:do_deploy virtual/bootloader:do_deploy ${@bb.utils.contains('MACHINE_FEATURES', 'optee', 'virtual/optee-os:do_deploy', '',d)} python3-nuwriter-native:do_install mtd-utils-native:do_populate_sysroot jq-native:do_populate_sysroot"
...
python do_deploy() {
    ...
    if d.getVar('SECURE_BOOT') == "no":
        ...
        subprocess.run(f"rm -rf $(date '+%m%d-*')", shell=True, check=True)
        if os.path.exists(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),"enc_bl2-ma35d0-spinand.dtb")):
            os.remove(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),"enc_bl2-ma35d0-spinand.dtb"))
            os.remove(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),"enc_bl2-ma35d0-spinand.bin"))
    else:
        ...
        shutil.copy("conv/enc_bl2-ma35d0.dtb","enc_bl2-ma35d0-spinand.dtb")
        shutil.copy("conv/enc_bl2-ma35d0.bin","enc_bl2-ma35d0-spinand.bin")
        ...
        pack_data["image"][1]["file"]="enc_bl2-ma35d0-spinand.dtb"
        pack_data["image"][2]["file"]="enc_bl2-ma35d0-spinand.bin"
